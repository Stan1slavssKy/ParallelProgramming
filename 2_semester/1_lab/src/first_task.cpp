...
void MPIParallelization(double **subarrays, int nmb_orig_to_calc);
void GatheringAllWork(double **subarrays, double *main_array, int nmb_orig_to_calc, int rank, int start, int i_elems_per_array);
...
int main(int argc, char **argv) {
    ...
    int i_elems_per_array = JSIZE * (JSIZE / commsize + 1);
    ...
    for (int i = 0; i < nmb_orig_to_calc; ++i) {
        double *current_array = new double[i_elems_per_array];
    ...
    MPIParallelization(subarrays, nmb_orig_to_calc);

    if (commsize > 1) {
        GatheringAllWork(subarrays, main_array, nmb_orig_to_calc, rank, start, i_elems_per_array);
    }
...
void MPIParallelization(double **subarrays, int nmb_orig_to_calc)
{
    for (int arr_idx = 0; arr_idx < nmb_orig_to_calc; ++arr_idx) {
        ...
        for (size_t counter = 0; counter < (ISIZE / 8); ++counter) {
